...
def guardar_registro(registro, archivo):
    """Guarda el registro en un archivo CSV."""
    with open('registro_usuarios.cvs', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Nombre", "Nombre de usuario", "Identificador", "Contraseña"])
        for usuario in registro.values():
            writer.writerow([usuario.nombre, usuario.nombre.lower().replace(" ", ""), usuario.identificador, usuario.contraseña])

def cargar_registro(archivo):
    """Carga el registro desde un archivo CSV."""
    registro = {}
    with open('registro_usuarios.cvs', 'r') as file:
        reader = csv.DictReader(file)
        for row in reader:
            registro[row["Nombre"]] = {
                "nombre": row["Nombre"],
                "nombre_usuario": row["Nombre de usuario"],
                "identificador": row["Identificador"],
                "contraseña": row["Contraseña"]
            }
    return registro

def serializar_registro(registro, archivo):
    """Serializa el registro y lo guarda en un archivo JSON."""
    with open('registro_usuarios.json', 'w') as file:
        json.dump(registro, file)

def deserializar_registro(archivo):
    """Deserializa el registro desde un archivo JSON."""
    with open('registro_usuarios.json', 'r') as file:
...
def main():
    ...
    # Guardar el registro en un archivo CSV
    registro = manejador_usuarios.usuarios
    guardar_registro(registro, 'registro_usuarios.csv')

    # Serializar y guardar el registro en un archivo JSON
    serializar_registro(registro, 'registro_usuarios.json')
